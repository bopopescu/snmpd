...
1. create MIB Py file and copy it to MIBSPY directory
   build-pysnmp-mib -o ../MIBSPY/AXON-MIB.py AXON-MIB.mib

2. sudo python <script_name>

3. Execute snmpwalk command (install snmp package for snmpwalk)
   snmpwalk -m AXON-MIB.py -v 2c -c public localhost .1
   snmpwalk -m ./AXON-MIB.mib -v 2c -c public 127.0.0.1 AXON-MIB::digiEsSnmpOidModel

   snmpwalk -m ./AXON-MIB.mib -v 2c -c public 127.0.0.1 .1
AXON-MIB::digiSnmpOidModel.0 = STRING: "Error"
AXON-MIB::digiSnmpOidType.0 = STRING: "Axon410"
AXON-MIB::digiEsSnmpOidModel.0 = STRING: "DataCaptor Terminal Server 100 Series"
AXON-MIB::digiEsSnmpOidModel.0 = No more variables left in this MIB View (It is past the end of the MIB tree)
...
all
--------------
Write/Read Table enteries for ports

http://snmplabs.com/pysnmp/examples/smi/agent/implementing-mib-objects.html#agent-operations-on-mib
https://sourceforge.net/p/pysnmp/mailman/message/26148849/
...
#PIDFILE = path.join(getcwd(), PIDFILE)
...
class SNMPAgent(object):
    ...
    def __init__(self):
        ...
        pub = path.join(getcwd(),'scripts/snmpd', 'PYSNMP_MIBS')
        priv = path.join(getcwd(),'scripts/snmpd', 'MIBSPY')
...
def logger():
    ...
    #handle = logging.handlers.RotatingFileHandler(fp,
    #                                            maxBytes=LOG_SIZE,
